...
- **Profile is not the whole envelope — frame rate, level and VBV peaks matter too.** A clip can be Main/yuv420p and still freeze the box mid-playback. Prod, 2026-09: `IMG_0053.MP4` was Main/yuv420p but **1920×1080 @ 54.5 fps, level 4.2**, with an unconstrained VBV that **peaked at 11 Mbps** (avg 2.4) around t≈198 s — and the TV froze in exactly that stretch. iPhone clips are 55–60 fps and the transcoder used to pass frame rate straight through (`-r` was never set) while bare `-crf` left peak bitrate unbounded. `QUALITY_PRESETS` now carry `maxFps: 30` plus `maxrate`/`bufsize` (capped CRF), and **no explicit `-level` is passed** — x264 derives the minimum level once fps/resolution are capped (720p30 → 3.1), whereas pinning 4.0 would only ever *raise* the declared level. The fps cap is applied **only when the source exceeds it** (an unreadable rate counts as "too high"), so a 24 fps source isn't padded with duplicate frames. `isKioskSafe` gates fps ≤30 and level ≤4.0 as well. **Note the standing tension:** the `high` preset still emits 1080p, which `isKioskSafe` rejects on resolution alone — `high` is deliberately outside the documented ≤720p envelope, and `standard` is the only preset whose output is fully kiosk-safe. When a box misbehaves on one clip, `standard` is the quick unblock.
