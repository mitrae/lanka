...
- **Server URL is compile-time**, not a runtime setting: `BuildConfig.LANKA_SERVER_URL` (default `http://lanka-server:3000`, a tailnet hostname). No in-app picker — rebuild to retarget: `./gradlew :app:assembleWebviewDebug -PLANKA_SERVER_URL=http://<host>:<port>` (a LAN IP for local box testing; `http://lanka-server:3000` for the tailnet). **The app is now flavored** — there is no plain `assembleDebug`; use `assembleWebviewDebug` or `assembleNativeDebug` (see "Native player flavor" below).
...
## Native player flavor (Lanka-vs / ExoPlayer)

A second Android flavor renders video with a **native ExoPlayer (Media3)** surface instead of the WebView's HTML5 `<video>` — added because the WebView path can't decode H.264 High and is prone to renderer OOM on long runs. The `android/` project has a Gradle **`surface` flavor dimension**: `webview` (default → `ai.lanka.kiosk`, label "Lanka", the original kiosk, unchanged) and `native` (→ `ai.lanka.kiosk.vs` via `applicationIdSuffix=".vs"`, label "Lanka-vs"). Distinct `applicationId`s → both APKs install side-by-side on one box and register as two separate device rows. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-28-lanka-native-exoplayer-flavor*`.

- **Build per flavor** — there is no plain `assembleDebug` anymore: `./gradlew :app:assembleWebviewDebug` / `:app:assembleNativeDebug` (both honor `-PLANKA_SERVER_URL=…`). Unit tests: `testWebviewDebugUnitTest` / `testNativeDebugUnitTest`. APKs land under `app/build/outputs/apk/{webview,native}/debug/app-{webview,native}-debug.apk`.
- **Source-set layout:** shared kiosk/OTA/cache (`KioskActivity`, `KioskFlags`, `DevicePolicy`, `KioskLock`, `BootReceiver`, `OtaInstaller`, `MediaCache`, `DeviceId`) in `src/main`; WebView-only classes (`MainActivity`, `LankaWebViewClient`, `LankaChromeClient`, `NativeFSBridge`) in `src/webview`; the native player (`PlayerActivity` + `player/*.kt`: ManifestClient, Scheduler, PlaybackView, TelemetryClient, CommandClient/Dispatcher) in `src/native`. Each flavor manifest declares its own launcher Activity; shared permissions/receivers live in `src/main/AndroidManifest.xml`. `BootReceiver` is flavor-agnostic (`getLaunchIntentForPackage`) so both flavors self-start on boot.
- **The `native` flavor has NO WebView in the playback path** — Kotlin owns the brain (a 1:1 port of the web player composables: `useReconciler`/`createPlayerScheduler`/`useTelemetry`/`useCommandChannel`). It speaks the same unchanged control plane (manifest GET + SSE `/stream`, telemetry POST, command `/ws`). Pure cores have JVM unit tests mirroring the vitest cases; transport/UI are build-verified.
- **Native plays cached files directly** via `Uri.fromFile(MediaCache.file(sha))` — none of the WebView's `MediaCache.intercept`/Range/sniff machinery is used (that exists only because an http-origin WebView can't load `file://`). `ManifestClient` pre-downloads all manifest media before binding; ExoPlayer streams the `/media/:sha` proxy URL on a cache miss. **Images must be pre-cached** — the ImageView path only decodes local files (no network fallback; a miss errors → self-heal advances).
- **Native-only unit tests MUST live in `src/testNative/`** (registered via `getByName("testNative") { java.srcDirs(...) }` in `app/build.gradle.kts`), NOT the shared `src/test/`. Putting them in `src/test` compiles them into *both* flavors' unit tests, and `testWebviewDebugUnitTest` then fails (the webview flavor lacks the `src/native` classes). Shared-class tests (MediaCacheTest, OtaInstallerTest) stay in `src/test`.
- **`media3`/`okhttp`/`kotlinx-serialization` are `nativeImplementation`-scoped** so the WebView APK doesn't grow. `kotlinx-serialization-json` is also a `testImplementation`.
- **Fleet awareness:** the native APK reports `surface: "native"` on register/telemetry (`devices.surface` column, default `'webview'`); the dashboard device detail shows a Native/WebView badge. APK releases carry a `flavor` tag (`apk_releases.flavor`) — currently stored but **not yet used to gate** the OTA picker by device surface (deferred follow-on; a `native` box can still be sent a `webview` APK).
- **On-box verification is the real gate** — playback, the ~120ms A/B crossfade (TextureView alpha; documented hard-cut fallback if too costly on low-end boxes), the command channel, and native OTA are **build-verified only**; test on a physical box against a **production build** (`pnpm build` + `node .output/server/index.mjs`), not `pnpm dev`. Launch `ai.lanka.kiosk.vs/ai.lanka.kiosk.PlayerActivity`.
...
- **Android:** `./gradlew test` (in `android/`) runs both flavors' JVM unit tests; per-flavor `testWebviewDebugUnitTest` / `testNativeDebugUnitTest`. Native-only tests live in `src/testNative/` (see "Native player flavor").
