...
describe('sessions', () => {
  ...
  it('round-trips a session token to a user', async () => {
    const u = await seedUser(db, { email: 'admin', role: 'admin' })
    const token = await createSession(db, u.id)
    const su = await getSessionUser(db, token)
    expect(su).toMatchObject({ id: u.id, email: 'admin', role: 'admin', organizationId: null })
  })
  ...
  it('returns null for an expired session', async () => {
    const u = await seedUser(db, { email: 'a', role: 'admin' })
